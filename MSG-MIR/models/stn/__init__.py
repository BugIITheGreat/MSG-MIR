...
from .local_stn import LocalSTN
...
def modify_commandline_options(parser, is_train=True):
    ...
    parser.add_argument('--stn_type', type=str, default='local',
                        help='The type of STN to use. Currently supported are [unet, affine, local]')
    if is_train:
...
def define_stn(opt, stn_type='affine'):
    ...
    if stn_type == 'local':
        stn = LocalSTN(nc_a, nc_b, height, width, cfg, opt.init_type, opt.stn_bilateral_alpha,
                      (not opt.stn_no_identity_init), opt.stn_multires_reg)
