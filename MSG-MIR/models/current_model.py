...
class NEMARModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_anti', type=float, default=0.0)
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionCOS = torch.nn.MSELoss()
    ...
    def setup_visualizers(self):
        ...
        loss_names_A = ['L1_TR', 'GAN_TR', 'L1_RT', 'GAN_RT', 'smoothness',
                        'D_fake_TR', 'D_fake_RT', 'D', 'A_RT', 'A_TR']
    ...
    def forward(self):
        ...
        wraped_images, reg_term, grid = self.netR(self.real_A, self.real_B, apply_on=[self.real_A, self.fake_B])
    ...
    def backward_T_and_R(self):
        ...
        # ----> Reconstruction loss:
        self.loss_L1_TR = self.opt.lambda_recon * torch.sqrt(self.criterionMSE(self.fake_TR_B, self.real_B))
        # ----> Anti-Identity loss:
        self.loss_A_TR = -self.opt.lambda_anti * torch.sqrt(self.criterionMSE(self.fake_B, self.real_B))
        # ----> GAN loss:
        ...
        # ----> Anti-Identity loss:
        self.loss_A_RT = -self.opt.lambda_anti * torch.sqrt(self.criterionMSE(self.registered_real_A, self.real_A))
        ...
        loss = self.loss_L1_TR + self.loss_L1_RT + self.loss_GAN_TR + \
               self.loss_GAN_RT + self.loss_smoothness + \
               self.loss_A_RT + self.loss_A_TR
        loss.backward()
