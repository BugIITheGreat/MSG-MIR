...
class OnlySTNModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_smooth', type=float, default=1000.0, help='Regularization term used by the STN')
            parser.add_argument('--enable_tbvis', action='store_true', default=True,
    ...
    def setup_visualizers(self):
        ...
        loss_names_A = ['L1', 'smoothness']

        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        # visual_names_A = ['registered_real_A', 'fieldDone', 'refield1', 'refield2', 'refield3', 'refield4',
        #                   'refield5', 'refield6', 'refield7', 'homofieldDone']

        visual_names_A = ['registered_real_A', 'fieldDone']
    ...
    def forward(self):
        ...
        wraped_images, reg_term = self.netR(self.real_A, self.real_B, apply_on=[self.real_A, self.real_B])
        with torch.no_grad():
            ...
            print(self.deformation_field_A_to_B.shape)
            default_map = torch.tensor(np.ones((self.deformation_field_A_to_B.shape[1],
                                                self.deformation_field_A_to_B.shape[2])),
                                       dtype=torch.float).unsqueeze(0).unsqueeze(-1).to('cuda:0')
            self.fieldDone = torch.cat((default_map, self.deformation_field_A_to_B), -1).permute(0,-1,1,2)
            # default_map = torch.tensor(np.ones((self.homofield.shape[1],
            #                                     self.homofield.shape[2])),
            #                            dtype=torch.float).unsqueeze(0).unsqueeze(-1).to('cuda:0')
            # self.homofieldDone = torch.cat((default_map, self.homofield), -1).permute(0, -1, 1, 2)
            # for i, index in enumerate(self.all_field.values()):
            #     default_map = torch.tensor(np.ones((index.shape[1], index.shape[2])),
            #                                dtype=torch.float).unsqueeze(0).unsqueeze(-1).to('cuda:0')
            #     self.refields.append(torch.cat((default_map, index), -1).permute(0, -1, 1, 2))
            # self.refield1 = self.refields[0]
            # self.refield2 = self.refields[1]
            # self.refield3 = self.refields[2]
            # self.refield4 = self.refields[3]
            # self.refield5 = self.refields[4]
            # self.refield6 = self.refields[5]
            # self.refield7 = self.refields[6]
            # self.fieldDone = transforms.ToPILImage((gridi.squeeze(0)))
            self.offset_cnt += 1
    ...
    def backward_T_and_R(self):
        ...
        # ----> Reconstruction loss:
        self.loss_L1 = self.opt.lambda_recon * torch.sqrt(self.criterionMSE(self.registered_real_A, self.real_B))
        # ----> GAN loss:
        ...
        # Translation First:
        # ----> Reconstruction loss:
        # self.loss_L1_RT = self.opt.lambda_recon * torch.sqrt(self.criterionMSE(self.fake_RT_B, self.real_B))
        ...
        # self.grad_registered_real_A = gradient_1order(self.registered_real_A)
        # self.grad_real_B = gradient_1order(self.real_B)
        # self.grad_term = torch.sqrt(self.criterionCOS(self.grad_registered_real_A, self.grad_real_B))
        self.loss_smoothness = self.opt.lambda_smooth * self.stn_reg_term
        # self.loss_grad = self.opt.lambda_grad * self.grad_term

        loss = self.loss_L1 + self.loss_smoothness
        loss.backward()
